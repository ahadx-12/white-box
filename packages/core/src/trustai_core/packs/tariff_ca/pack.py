...
from trustai_core.packs.tariff_ca.prompts import (
...
class TariffPack:
    ...
    async def run(self, input_text: str, options: dict[str, object] | None) -> TariffVerificationResult:
        ...
        duty_calculator = CADutyCalculator()
...
def _default_flow() -> DutyFlow:
    return DutyFlow(importing_country="CA")
...
def _resolve_flow(input_text: str) -> DutyFlow:
    ...
    return DutyFlow(
        importing_country=flow_payload.get("importing_country", "CA"),
...
def _evidence_root() -> Path:
    ...
    return root / "tariff_ca" / "evidence" / "sources"
...
def _ensure_candidate_coverage(
    ...
    section_prefixes.update({f"CA.{section}" for section in sections if section})
...
register_pack("tariff_ca", lambda context: TariffPack(context))
