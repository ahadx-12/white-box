...
def test_order_sensitivity():
    ...
    atom_ab = AtomModel(subject="a", predicate="kills", obj="b", is_true=True)
    atom_ba = AtomModel(subject="b", predicate="kills", obj="a", is_true=True)
...
def test_manifest_shuffle_invariance():
    ...
    encoder = AtomEncoder(memory)
    atoms = [
        AtomModel(subject="door", predicate="state", obj="open", is_true=True),
        AtomModel(subject="door", predicate="color", obj="red", is_true=True),
    ]
